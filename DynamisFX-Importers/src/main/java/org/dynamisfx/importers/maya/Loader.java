...
import org.dynamisfx.importers.SmoothingGroups;
...
import org.dynamisfx.importers.maya.parser.MParser;
import org.dynamisfx.importers.maya.values.MArray;
import org.dynamisfx.importers.maya.values.MBool;
...
import org.dynamisfx.importers.maya.values.MInt;
...
import org.dynamisfx.importers.maya.values.MString;
...
import org.dynamisfx.utils.geom.Joint;
import org.dynamisfx.utils.geom.Vec3f;
...
class Loader {
    ...
    protected void processClusterType(MNode n) {
        ...
        List<Joint> jointNodes = new ArrayList<>();
        Set<Parent> jointForest = new HashSet<>(); // root's children that have joints in their trees
        ...
        for (MPath path : toPaths) {
            ...
            if (toNode.isInstanceOf(transformType)) {
                ...
                if (to instanceof MayaGroup) {
                    ...
                    if (null != s) {// if (DEBUG) System.out.println("selector = " + s);
                        switch (s) {
                            case "t[0]":
                                ref = g.t.xProperty();
                                break;
