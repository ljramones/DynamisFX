...
/**
 * Tests for OffImporter.
 */
class OffImporterTest {

    private OffImporter importer;

    @BeforeEach
    void setUp() {
        importer = new OffImporter();
    }

    @Test
    void testExtensions() {
        assertTrue(importer.isSupported("off"));
        assertTrue(importer.isSupported("OFF"));
    ...
    void testLoad() throws Exception {
        Model3D model = importer.load(getClass().getResource("cube.off"));
    ...
    void testLoadAsPoly() throws Exception {
        // OFF returns same as load
        Model3D model = importer.loadAsPoly(getClass().getResource("cube.off"));
